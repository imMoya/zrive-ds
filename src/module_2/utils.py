...
def map_ordered_items_to_prices(
	df: pd.DataFrame, inventory_df: pd.DataFrame, list_col_name: str
):
...
def extract_hour_minute(
	df: pd.DataFrame, datetime_col: str, hour_min_col: str
) -> pd.DataFrame:
	df[datetime_col] = pd.to_datetime(df[datetime_col])
	df[hour_min_col] = pd.to_datetime(
		df[datetime_col].dt.strftime('%H:%M'), format='%H:%M'
	).dt.time
	return df
